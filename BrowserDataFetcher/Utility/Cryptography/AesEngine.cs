namespace BrowserDataFetcher
{
    ...
    /// <summary>
    /// The Advanced Encryption Standard engine.
    /// </summary>
    public class AesEngine : IBlockCipher
    {
        /// <summary>
        /// The forward substitution box.
        /// </summary>
        ...
        /// <summary>
        /// Vector used in calculating key schedule.
        /// </summary>
        ...
        /// <summary>
        /// Computation table for calculating rounds.
        /// </summary>
        ...
        /// <summary>
        /// Calculate the necessary round keys. The number of calculations depends on key size and block size.
        ...
        private uint[][] GenerateWorkingKey(byte[] key, bool forEncryption)
        {
            ...
            int kc = keyLen >> 2;
            this._rounds = kc + 6;  // This is not always true for the generalized Rijndael that allows larger block sizes.
        ...
        /// </summary>
        /// <param name="forEncryption">
        /// Value indicating whether or not we are for encryption.
        /// </param>
        /// <param name="parameters">
        /// The parameters required to set up the cipher.
        /// </param>
        /// <exception cref="ArgumentException">
        /// The <see cref="ArgumentException"/> thrown if the parameters argument is inappropriate.
        /// </exception>
        public virtual void Init(bool forEncryption, ICipherParameters parameters)
        {
            ...
            this._s = GeneralTools.Clone(forEncryption ? S : Si);
        }

        /// <summary>
        /// Gets the name of the algorithm the cipher implements.
        /// </summary>
        ...
        /// <summary>
        /// Gets a value indicating whether the partial block is okay or not.
        /// </summary>
        ...
        /// <summary>
        /// Gets block size for this cipher (in bytes).
        /// </summary>
        /// <returns>
        /// An <see cref="Int32"/> indicating the block size.
        /// </returns>
        ...
        /// </param>
        /// <returns>
        /// An <see cref="Int32"/> indicating the number of bytes processed and produced.
        /// </returns>
        /// <exception cref="InvalidOperationException">
        /// The <see cref="InvalidOperationException"/>.
        /// </exception>
        public virtual int ProcessBlock(byte[] input, int inOff, byte[] output, int outOff)
        {
            ...
            GeneralTools.CheckDataLength(input, inOff, 16, "input buffer too short");
            GeneralTools.CheckOutputLength(output, outOff, 16, "output buffer too short");
        ...
        /// <summary>
        /// Encrypts a block. The final round's table is a simple function of S so we don't use a whole other four tables for it.
        /// </summary>
        /// <param name="KW">
        /// The secret key.
        /// </param>
        ...
        {
            uint[] kw = KW[0];
            uint t0 = this._c0 ^ kw[0];
            uint t1 = this._c1 ^ kw[1];
            uint t2 = this._c2 ^ kw[2];

            uint r0, r1, r2, r3 = this._c3 ^ kw[3];
            int r = 1;
            while (r < _rounds - 1)
            {
                kw = KW[r++];
            ...
            kw = KW[r];
            this._c0 = (uint)S[r0 & 255] ^ (((uint)S[(r1 >> 8) & 255]) << 8) ^ (((uint)_s[(r2 >> 16) & 255]) << 16) ^ (((uint)_s[(r3 >> 24) & 255]) << 24) ^ kw[0];
            this._c1 = (uint)_s[r1 & 255] ^ (((uint)S[(r2 >> 8) & 255]) << 8) ^ (((uint)S[(r3 >> 16) & 255]) << 16) ^ (((uint)_s[(r0 >> 24) & 255]) << 24) ^ kw[1];
            this._c2 = (uint)_s[r2 & 255] ^ (((uint)S[(r3 >> 8) & 255]) << 8) ^ (((uint)S[(r0 >> 16) & 255]) << 16) ^ (((uint)S[(r1 >> 24) & 255]) << 24) ^ kw[2];
            this._c3 = (uint)_s[r3 & 255] ^ (((uint)_s[(r0 >> 8) & 255]) << 8) ^ (((uint)_s[(r1 >> 16) & 255]) << 16) ^ (((uint)S[(r2 >> 24) & 255]) << 24) ^ kw[3];
        }

        /// <summary>
        /// Decrypts a block. The final round's table is a simple function of Si so we don't use a whole other four tables for it.
        /// </summary>
        /// <param name="KW">
        /// The secret key.
        /// </param>
        private void DecryptBlock(uint[][] KW)
        {
            ...
            kw = KW[0];
            this._c0 = (uint)Si[r0 & 255] ^ (((uint)_s[(r3 >> 8) & 255]) << 8) ^ (((uint)_s[(r2 >> 16) & 255]) << 16) ^ (((uint)Si[(r1 >> 24) & 255]) << 24) ^ kw[0];
            this._c1 = (uint)_s[r1 & 255] ^ (((uint)_s[(r0 >> 8) & 255]) << 8) ^ (((uint)Si[(r3 >> 16) & 255]) << 16) ^ (((uint)_s[(r2 >> 24) & 255]) << 24) ^ kw[1];
            this._c2 = (uint)_s[r2 & 255] ^ (((uint)Si[(r1 >> 8) & 255]) << 8) ^ (((uint)Si[(r0 >> 16) & 255]) << 16) ^ (((uint)_s[(r3 >> 24) & 255]) << 24) ^ kw[2];
            this._c3 = (uint)Si[r3 & 255] ^ (((uint)_s[(r2 >> 8) & 255]) << 8) ^ (((uint)_s[(r1 >> 16) & 255]) << 16) ^ (((uint)_s[(r0 >> 24) & 255]) << 24) ^ kw[3];
        }
    }
}